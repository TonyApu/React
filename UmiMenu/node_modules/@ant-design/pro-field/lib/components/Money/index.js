...
var getTextByLocale = function getTextByLocale(locale, paramsText, precision, config) {
  ...
  try {
    ...
    var finalMoneyText = new Intl.NumberFormat(supportFormat && locale !== false ? (locale === null || locale === void 0 ? void 0 : locale.replace('_', '-')) || 'zh-Hans-CN' : 'zh-Hans-CN', (0, _objectSpread2.default)((0, _objectSpread2.default)({}, intlMap[locale || 'zh-Hans-CN'] || defaultMoneyIntl), {}, {
    ...
    // 是否有金额符号，例如 ¥ $
    var hasMoneySymbol = locale === false;
    ...
    if (['+', '-'].includes(operatorSymbol)) {
      ...
      return "".concat(moneySymbol || '').concat(operatorSymbol).concat(finalMoneyText.substring(hasMoneySymbol ? 2 : 1));
    }

    // 没有正负符号截取一位
    return "".concat(moneySymbol || '').concat(finalMoneyText.substring(hasMoneySymbol ? 1 : 0));
  } catch (error) {
    return moneyText;
  }
};
